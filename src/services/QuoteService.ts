...
function getExchangeApiData(baseCurrency: string): Promise<AxiosResponse> {
    console.log(`Requested currencies not in cache. Using API quota for exchange-api:`);
    return axios.get(`${exchange_url}${baseCurrency}`);
}

function calculateQuoteAmount(amount: number, rate: number): number {
    return amount * rate;
}

function setCache(baseCurrency: string, quoteCurrency: string, exchangeRate: number): void {
